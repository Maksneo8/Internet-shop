  ...
  <div>
    ...
    <div v-if="filteredProducts.length" class="product-grid">
      <div v-for="product in filteredProducts" :key="product.id" class="product-card border rounded-md p-4 mb-4">
        <img v-if="product.image && product.image !== 'undefined'" :src="product.image" alt="Product Image" class="w-full h-auto mb-4" />
        <h3 v-if="product.name" class="text-xl font-bold mb-2">{{ product.name }}</h3>
        <p v-if="product.description && product.description !== 'undefined'" class="mb-2">{{ product.description }}</p>
